pipeline {                                                // Mendefinisikan blok pipeline untuk CI/CD Jenkins
    ...
    tools {                                              // Mendefinisikan tools yang dibutuhkan untuk pipeline
        go 'Go 1.19'                                     // Menggunakan Go versi 1.19 yang sudah dikonfigurasi di Jenkins
    }
    ...
    environment {                                        // Mendefinisikan variabel lingkungan untuk pipeline
        ...
        APP_NAME = 'karsajobs'                          // Nama aplikasi untuk container Docker
    ...
    stages {                                           // Mendefinisikan tahapan-tahapan dalam pipeline
        ...
        stage('Test App') {                             // Tahap menjalankan unit test aplikasi Go
            steps {
                sh 'go test -v -short --count=1 $(go list ./...)' // Menjalankan semua test dengan verbose output
            }                                           // --count=1: tidak menggunakan cache test, go list ./...: semua package
        }
        
        stage('Docker Build') {                         // Tahap membangun image Docker
            steps {
        ...
        always {                                        // Tindakan yang selalu dijalankan, baik sukses maupun gagal
            ...
            withEnv([                                   // Menetapkan variabel lingkungan untuk blok shell
                ...
                sh '''                                  
                    ...
                    docker builder prune -f || true             // Membersihkan cache builder Docker
